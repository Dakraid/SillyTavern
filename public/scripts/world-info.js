...
export async function checkWorldInfo(
    ...
    while (scanState) {
        ...
        if (activatedNow.size > 1) {
            const sortedEntriesIndex = new Map(
                sortedEntries.map((entry, index) => [entry, index]),
            );
            newEntries = [...activatedNow].sort((a, b) => {
                ...
                return (
                    isBSticky - isASticky ||
                    (sortedEntriesIndex.get(a) ?? -1) -
                        (sortedEntriesIndex.get(b) ?? -1)
                );
            });
        } else {
