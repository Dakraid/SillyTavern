...
const processCharacter = async (item, directories, { shallow }) => {
    try {
        ...
        character.create_date =
			...
			new Date(Math.round(charStat.ctimeMs)).toISOString();
...
async function importFromCharX(uploadPath, { request }, preservedFileName) {
    ...
    const { card, avatar, auxiliaryAssets, extractedBuffers } =
		await parser.parse();
    ...
    const fileName =
		preservedFileName ||
		getPngName(processedCard.name, request.user.directories);
    ...
    if (auxiliaryAssets.length > 0) {
        try {
            ...
            console.warn(
                `CharX: Failed to persist auxiliary assets for ${characterFolder}`,
                error,
            );
        }
    }
...
async function importFromByaf(uploadPath, { request }, preservedFileName) {
    ...
    if (!preservedFileName) {
        ...
        for (const icon of byafData.images.slice(1)) {
            ...
            if (!fs.existsSync(altImagesFolder))
                fs.mkdirSync(altImagesFolder, { recursive: true });
            ...
            const file = getUniqueName(
                ...
                (name) =>
                    fs.existsSync(path.join(altImagesFolder, `${name}${extension}`)),
            );
            if (Buffer.isBuffer(icon.image)) {
                ...
                console.log(
                    `Created ${file}${extension} alternate icon from BYAF import`,
                );
            }
        }
    }
...
async function importFromJson(uploadPath, { request }, preservedFileName) {
    ...
    if (jsonData.spec !== undefined) {
        ...
        const pngName =
			preservedFileName || getPngName(jsonData.name, request.user.directories);
    ...
    async function (request, response) {
        try {
            ...
            if (!request.body || !request.body.avatar_url) {
                return response
                    .status(400)
                    .send('Error: no avatar_url in request body');
            }
            ...
            if (!fs.existsSync(characterPath)) {
                return response
                    .status(400)
                    .send('Error: character file does not exist');
            }
            const data = await readCharacterData(characterPath);
            if (!data) {
                return response
                    .status(400)
                    .send('Error: failed to read character data');
            }
...
async function mergeCharacterUpdate(
    ...
    if (!validator.validate()) {
        return {
            ok: false,
            error: validator.lastValidationError ?? 'Validation failed',
        };
    }
...
router.post('/generate-voronoi-composite', async function (request, response) {
    try {
        if (!request.body || !Array.isArray(request.body.avatars)) {
            return response.status(400).send({ message: 'avatars must be an array' });
        }
        ...
        if (avatars.length === 0) {
            return response
                .status(400)
                .send({ message: 'At least one avatar is required' });
        }
        ...
        for (const avatar of avatars) {
            ...
            if (!avatar.endsWith('.png')) {
                return response
                    .status(400)
                    .send({ message: `Invalid avatar file type: ${avatar}` });
            }
...
router.get('/generate-voronoi-composite', async function (request, response) {
    try {
        ...
        if (!file || typeof file !== 'string') {
            return response
                .status(400)
                .send({ message: 'file parameter is required' });
        }

        const basename = path.basename(file);
        if (basename !== file || file.includes('..')) {
            return response.status(400).send({ message: 'Invalid file parameter' });
        }
        ...
        return response
            .status(500)
            .send({ message: 'Failed to serve composite image' });
    }
});
